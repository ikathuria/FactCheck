...
without live Redis or LLM quota: two identical requests run the pipeline once,
...
def test_second_identical_request_is_cached(monkeypatch):
    ...
    monkeypatch.setattr(router.redis_cache, "get", lambda k: fake_store.get(k))
    monkeypatch.setattr(
        router.redis_cache, "set", lambda k, v, ttl_seconds=0: fake_store.__setitem__(k, v)
    )
...
def test_modules_importable():
    ...
    assert isinstance(redis_cache.is_configured(), bool)

"""POST /verify endpoint with Turso caching + Turso history."""
...
from ..cache import keys, turso_cache
...
def verify(request: VerifyRequest) -> VerifyResponse:
    """Run the 5-agent fact-check pipeline, with 7-day Turso caching.

    On cache hit: return the stored result with cached=true.
    On miss: run the pipeline, store in the Turso cache (7-day TTL), record a
    summary row in the Turso `searches` table, and return cached=false.
    """
    key = keys.cache_key(request.claim, request.source_mode)

    cached = turso_cache.get(key)
    ...
    result.ttl_expires_at = now + timedelta(seconds=turso_cache.DEFAULT_TTL_SECONDS)

    # Best-effort persistence (both layers no-op when not configured).
    turso_cache.set(key, result.model_dump(mode="json"))
    store.insert_search(
        claim=result.claim,
        claim_hash=key,

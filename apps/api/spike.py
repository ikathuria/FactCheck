...
def critic_agent(state: PipelineState) -> dict:
    ...
    prompt = f"""You are an adversarial critic reviewing a fact-check. Your job: catch overconfident or unsupported verdicts.
...
{{
  "verdict": "supported" | "refuted" | "contested" | "insufficient_evidence",
  "confidence": <float 0.0 to 1.0>,
  "reasoning": "<2-4 sentences — explain if you changed anything and why>"
}}

Override rules (must apply strictly):
1. If the evidence does NOT clearly support the synthesis verdict -> lower confidence or change verdict
2. If sources contradict each other significantly -> verdict MUST be "contested"
...
5. If synthesis looks correct and well-supported -> keep it (your job is to catch errors, not introduce doubt)"""
